...
class InstructionTest extends FlatSpec {
  ...
  def testContext(rovers: Seq[Rover]) = Context(testPlateau, rovers)
  
  "the turn left instruction" should "ignore the plateau and other rovers and turn the rover to the left" in {
    assert(Instruction.TurnLeft(null)(testRover) === testRover.turnLeft)
  }
  
  "the turn right instruction" should "ignore the plateau and other rovers and turn the rover to the right" in {
    assert(Instruction.TurnRight(null)(testRover) === testRover.turnRight)
  }
  
  "the move instruction" should "only allow the rover to move if there is room on the plateau" in {
    assert(Instruction.Move(Context(Plateau(Location(3, 4)), Seq()))(testRover) === testRover)
    assert(Instruction.Move(testContext(Seq()))(testRover) === testRover.move)
  }
  
  it should "only allow the rover to move if there isn't another rover in the new spot" in {
    assert(Instruction.Move(testContext(Seq(Rover(Location(4, 3), Orientation.North))))(testRover) === testRover)
    assert(Instruction.Move(testContext(Seq(Rover(Location(4, 4), Orientation.North))))(testRover) === testRover.move)
  }
}
